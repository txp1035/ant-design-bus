...
import type { BaseParams, Range } from './transform';
...
export function translateBase(params: string, type: string, { appointMode, stepMode, baseMode }: { appointMode; stepMode; baseMode }) {
  const obj = transformBase(params) as BaseParams;
  ...
  const newSteps: Record<string, Range[]> = {};
...
export function translateDay(params: string, translateMode) {
  if (translateMode.day) {
    const value = translateMode.day(params);
    if (value !== false) {
      return value;
    }
  }
...
export function translateWeek(params: string, translateMode) {
  if (translateMode.week) {
    const value = translateMode.week(params);
    if (value !== false) {
      return value;
    }
  }
  ...
  if (appointReg.test(params)) {
    return `第${(appointReg.exec(params) as string[])[2]}周的周${(appointReg.exec(params) as string[])[1]}`;
  }
  const lastReg = /^([1-7])L$/;
...
const defaultOptions = {
  ...
  translateMode: {
    appointMode: ({ cur, index, arr, type }) => {
      ...
      const curStr = `${getTimeValue(cur.start, appointUnit[type])}到${getTimeValue(cur.end, appointUnit[type])}${mark}`;
      return curStr;
    },
    stepMode: ({ curs, indexs, arrs, type, step, index, arr }) => {
      ...
      const curStr = `${getTimeValue(curs.start, appointUnit[type])}到${getTimeValue(curs.end, appointUnit[type])}每${step}${stepUnit[type]}${mark}`;
      return curStr;
    },
...
export default function translate(params: string, options: Options = {}) {
  const { translateEmum = defaultOptions.translateEmum, translateMode = defaultOptions.translateMode } = options;
  ...
  const newTranslateMode = { ...defaultOptions.translateMode, ...translateMode };
  // 通用匹配
  const { second, minute, hour, day, moth, week, year } = obj;
  return newTranslateMode.allMode({
    second: second && translateSecond(second, newTranslateMode),
    minute: translateMinute(minute, newTranslateMode),
    hour: translateHour(hour, newTranslateMode),
    day: translateDay(day, newTranslateMode),
    moth: translateMoth(moth, newTranslateMode),
    week: translateWeek(week, newTranslateMode),
    year: year && translateYear(year, newTranslateMode),
  });
}
