import type { cronType } from './utils';
import { getCronType } from './utils';
/**
 * 描述 验证指定值和区间是否在指定范围内
 * @author ShawDanon
 * @date 2020-06-14
...
export function checkBase(
  ...
  list.forEach((item) => {
    ...
    if (regRange.test(item)) {
      isOther = false;
      // 验证区间开始和结束
      const start = Number((regRange.exec(item) as string[])[1]);
      const end = Number((regRange.exec(item) as string[])[2]);
      // 开始必须小于结束
      if (start > end) {
        isRight = false;
      }
      ...
      if (stepParams && stepReg.test(stepParams)) {
        const step = Number((stepReg.exec(stepParams) as string[])[1]);
        if (step > stepEnd || step < stepStart) {
          isRight = false;
        }
      }
    }
