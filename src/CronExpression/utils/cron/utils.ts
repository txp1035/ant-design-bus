 ...
 * @returns {any}
 */
export function getCronType(params: string): cronType | '' {
...
export function transformRange(params: (string | number | Range)[], length: number = 2) {
  ...
  const newArr = [...flatten(appointArr), ...rangeArr.map((item) => ({ start: item[0], end: item[item.length - 1] }))];
  return newArr;
}

// 通用工具类
export const arrToObj = (arr: (string | number)[]) => {
  const obj = {};
  arr.forEach((item) => {
    obj[item] = item;
  });
...
export function removal(list: (string | Record<string, string>)[], key?: string) {
  // 对象去重
  if (key) {
    return list.filter((item, index, arr) => typeof item === 'object' && arr.findIndex((childItem) => childItem[key] === item[key]) === index);
  }
  // 字符串去重
  return [...new Set(list)];
}
