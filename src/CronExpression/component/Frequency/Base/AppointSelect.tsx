...
export const disabledRange = ({ limitTop, limitBottom }: any) => {
...
const AppointSelect: React.FC<AppointProps> = ({
  ...
  const [name, setItemsName] = useState({
    limitTop: dataSourceList[0],
    limitBottom: dataSourceList[dataSourceList.length - 1],
  });
  // 增加区间item
  const addItem = () => {
    ...
    if (limitTop && limitBottom) {
      setItems({
        ...items,
        [`${limitTop}-${limitBottom}`]: `${dataSource[limitTop || '']}到${dataSource[limitBottom || '']}`,
      });
      setItemsName({
        limitTop: null,
        limitBottom: null,
