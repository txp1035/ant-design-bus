...
export type second = base
export type minute = base
export type hour = base
 ...
 * @returns {any}
 */
export function transformDay(params: string | day, isAppoint: boolean = false) {
  if (typeof params === 'string') {
    if (params === '?') {
  ...
  if (
    ...
    isLastDay === undefined &&
    (list === undefined || (list && list.length === 0))
  ) {
    if (isAppoint) {
      return '?';
    }
 ...
 * @returns {any}
 */
export function transformWeek(params: string | week, isAppoint: boolean = false) {
  if (typeof params === 'string') {
    if (params === '?') {
      return { isCommon: false, isAppoint: true };
    }
